...
def ha_sensitivity_shock(target, percent):
    # loop over shocks
    eoe_equities = []
    ...
    for shock in shocks:
        ...
        for idx, name in enumerate(bank_names):
            ...
            agent = Heuristic_Agent(target=target, percent=percent, name=name)
    ...
    path=os.path.join('/home/tonyairhe_gmail_com', str(target*100)+'_'+str(percent*100)+'_'+'ha_sensitivity_shock.txt')
